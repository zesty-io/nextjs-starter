export async function fetchZestyPage(url, getNavigation = true) {
  ...
  if(process.env.zesty.production == "" && process.env.zesty.stage == ""){
    return  {
      ...
      meta: {
        ...
        web: {
          seo_meta_title: 'Connection to Zesty.io Instance is Missing',
  ...
  // setup default error message is URL does not resolve to Zesty data
  let data = {
    error: true,
  };
  ...
  if (getNavigation == true) {
    data.zestyNavigationTree = await buildJSONTreeFromNavigation(zestyURL,process.env.zesty.stage_password);
  }
...
async function buildJSONTreeFromNavigation(zestyURL,zpw='') {
