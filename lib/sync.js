...
module.exports = {
  ...
  if (!nextJSConfig.env.hasOwnProperty('zesty')) {
    console.log(
      failMark +
        ' Missing Zesty env NextJS Config, please add the following to your nextjs.config.js',
    );
    console.log(chalk.green.bold('          env: {'));
    console.log(chalk.green.bold('             zesty: {'));
    console.log(
      chalk.green.bold(
        '                 "stage" : "", // stage domain to zesty instance',
      ),
    );
    console.log(
      chalk.green.bold(
        '                 "production" : "", // stage domain to zesty instance',
      ),
    );
...
async function createFiles(config) {
  ...
  if (!success) {
    console.log(
      `${successMark} Creating Directory for Zesty Models: ${zestyModelsDirectory}`,
    );
    success = await fs.promises.mkdir(zestyModelsDirectory, {
      recursive: true,
    });
  } else {
  ...
  let stageGQLEndpoint = formURLWithPassword(
    config.stage,
    '/-/gql/',
    config.stage_password,
  );
  ...
  console.log(
    `${successMark} ${data.models.length} Zesty Models Accessed. Creating missing files. `,
  );
  ...
  for (const model of models) {
    model.component_name = `${model.gqlModelName}`;
  ...
  if (headerExists) {
    console.log(
      `   ${warningMark} Skipping ${chalk.yellow.bold(
        'Header.js',
      )} already exists ${chalk.gray.bold(headerPath)}`,
    );
  } else {
    console.log(
      `   ${successMark} Creating Header.js to ${chalk.gray.bold(headerPath)}`,
    );
  ...
  if (footerExists) {
    console.log(
      `   ${warningMark} Skipping ${chalk.yellow.bold(
        'Footer.js',
      )} already exists ${chalk.gray.bold(footerPath)}`,
    );
  } else {
    console.log(
      `   ${successMark} Creating Footer.js to ${chalk.gray.bold(footerPath)}`,
    );
  ...
  if (slugExists) {
    console.log(
      `   ${warningMark} Skipping ${chalk.yellow.bold(
        '[...slug].js',
      )} already exists ${chalk.gray.bold(slugPath)}`,
    );
  } else {
    console.log(
      `   ${successMark} Creating Footer.js to ${chalk.gray.bold(slugPath)}`,
    );
  }
