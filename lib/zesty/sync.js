...
(async () => {
  ...
  const tasks = new Listr([
    ...
    {
      title: 'Fetched Zesty Style Variables',

      task: async (ctx) => {
        try {
          let env = JSON.parse(process.env.PRODUCTION);
          let zestyURL = env ? ctx.nextConfig.production : ctx.nextConfig.stage;
          const styles = fetch(
            `${zestyURL}/-/headless/style-variables.json?zpw=${ctx.nextConfig.stage_password}`,
          );
...
async function writeStyles(d) {
  ...
  try {
    ...
    //filtering name and value from the fetched style variables
    for (const { name: rn, value: val } of d) {
      styles[rn] = val;
    }

    //creates styleVariables.json file to .zesty folder with referenceName and values as content.
