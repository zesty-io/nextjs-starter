...
(async () => {
  ...
  const tasks = new Listr([
    ...
    },
    {
      ...
      skip: (ctx) => {
        if (ctx.nextConfig?.env?.zesty?.stage_password || ctx.nextConfig?.env?.zesty?.options?.skip_config_overwrite == true ) {
      ...
      task: async (ctx) => {
        try {
          ...
          if (settings && Array.isArray(settings.data)) {
            ...
            // NOTE: WebEngine does not add the mode setting until it is loaded at least once
    ...
    },
    {
      ...
      task: async (ctx) => {
        try {
          ...
          await writeNextConfig(ctx.nextConfig.env.zesty);
        } catch (error) {
          throw error;
        }
      },
...
module.exports = {
  ...
  env: {
      zesty: {
          instance_zuid: "${config.instance_zuid}", // zesty unique id of content instance
          stage: "${config.stage}", // e.g. https://XYZ-dev.webengine.zesty.io
          production: "${config.production}", // e.g. https://www.acme.com
...
async function createFiles(config) {
  ...
  for (const model of gqlJSON.models) {
    ...
    if(config.options.model_ignore_list.find(modelZUID => modelZUID == model.zuid) !== undefined) { 
      steps.push(
        Promise.resolve(`Ignored: ${model.label} [${model.zuid}] is set to be ignored in next.config.js`),
      );
      continue; 
    }
